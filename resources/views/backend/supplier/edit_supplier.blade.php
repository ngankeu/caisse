                                ...
                                <div class="page-title-box">
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">Modifier le fournisseur</a></li>
                                            
                                        </ol>
                                    </div>
                                    <h4 class="page-title">Modifier le fournisseur</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title -->
    ...
    <div class="tab-pane" id="settings">
        <form method="post" action="{{ route('supplier.update') }}" enctype="multipart/form-data">
           ...
           <input type="hidden" name="id" value="{{ $supplier->id }}"> 
            ...
            <h5 class="mb-4 text-uppercase"><i class="mdi mdi-account-circle me-1"></i> Modifier le fournisseur</h5>
  ...
  <div class="col-md-6">
        <div class="mb-3">
           ...
           <select name="type" class="form-select @error('type') is-invalid @enderror" id="example-select">
                    ...
                    <option value="Distributor" {{ $supplier->type == 'Distributeur' ? 'selected' : '' }}>Distributeur </option>
                    <option value="Whole Seller" {{ $supplier->type == 'Grossiste' ? 'selected' : '' }}>Grossiste </option>
                </select>
                 @error('type')
      <span class="text-danger"> {{ $message }} </span>
            @enderror
