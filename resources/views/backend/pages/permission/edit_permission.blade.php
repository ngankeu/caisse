                                ...
                                <div class="page-title-box">
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">Modifier l'autorisation</a></li>
                                            
                                        </ol>
                                    </div>
                                    <h4 class="page-title">Modifier l'autorisation</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title -->
...
<div class="card">
    ...
    <div class="tab-pane" id="settings">
        <form id="myForm" method="post" action="{{ route('permission.update') }}" enctype="multipart/form-data">
            ...
            <h5 class="mb-4 text-uppercase"><i class="mdi mdi-account-circle me-1"></i> Modifier l'autorisation</h5>
    ...
    <div class="col-md-6">
        ...
        <option value="employee" {{ $permission->group_name == 'employee' ? 'selected' : '' }}> Employé</option>
        ...
        <option value="salary" {{ $permission->group_name == 'salary' ? 'selected' : '' }}> Salaire </option>
        <option value="attendence" {{ $permission->group_name == 'attendence' ? 'selected' : '' }}> Présence </option>
        <option value="category" {{ $permission->group_name == 'category' ? 'selected' : '' }}> Catégorie </option>
        <option value="product" {{ $permission->group_name == 'product' ? 'selected' : '' }}> Article </option>
        <option value="expense" {{ $permission->group_name == 'expense' ? 'selected' : '' }}> Dépenses</option>
        <option value="orders" {{ $permission->group_name == 'orders' ? 'selected' : '' }}> Ordres</option>
